...
for (const categoria of categorias) {
  ...
  let txt = document.createTextNode(categoria);
  option.appendChild(txt);
  frmControles.categorias.append(option);  
}
for (const producto of catalogo.productos){
  let option = document.createElement('option');
  let txt = document.createTextNode(producto.nombreProducto);
  option.appendChild(txt);
  frmControles.productos.append(option);  
}
